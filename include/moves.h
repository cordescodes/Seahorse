...
struct Move {
    uint64_t start; // start bit board
    uint64_t dest; // destination bit board
    int chessman; // which piece is moving? 1 = pawn, 2 = rook, 3 = knight, 4 = bishop, 5 = queen, 6 = king
...
class Moves
{
    public:
        ...
        uint64_t makeMove(Move theMove, uint64_t chessmen, int type); // type: 1 = pawn, 2 = rook, 3 = knight, 4 = bishop, 5 = queen, 6 = king
